...
# We check which version was installed to use the correct command
PIHOLE_VERSION=$(pihole -v | grep "Pi-hole" | cut -d'v' -f2 | cut -d'.' -f1)

if [[ "$PIHOLE_VERSION" == "6" ]]; then
    # Command for Pi-hole v6
    execute_silent "Setting password (v6 detected)" "pihole setpassword $ADMIN_PASS"
else
    # Command for Pi-hole v5
    execute_silent "Setting password (v5 detected)" "pihole -a -p $ADMIN_PASS"
fi
echo ""
