...
# Version: 2.6 (Fix: Unbound Permissions)
# ==========================================
...
if [[ "$unbound_choice" == "y" || "$unbound_choice" == "Y" ]]; then
    ...
    # Asignamos el archivo al usuario 'unbound' para que pueda leerlo
...
EOF
    ...
    # Comprobación de que ha arrancado
    if systemctl is-active --quiet unbound; then
        echo -e "${GREEN}✅ Unbound installed and running.${NC}"
    else
        echo -e "${RED}❌ Unbound failed to start. Check logs: 'journalctl -u unbound'${NC}"
    fi
else
...
CURRENT_IP=$(ip -4 addr show $CURRENT_IF | grep -oP '(?<=inet\s)\d+(\.\d+){3}' | head -n1)
