...
# We include sqlite3 for database and wget for unbound root hints
...
if [[ "$list_choice" == "y" || "$list_choice" == "Y" ]]; then
    ...
    echo -e "${GRAY}⏭  Skipping advanced lists (Default lists kept).${NC}"
fi
# --------------------------------------
...
# --- UNBOUND SETUP (NUEVO) ---
echo ""
echo -e "${YELLOW} Step 4.5: Unbound Recursive DNS (Privacy)${NC}"
echo -e "${GRAY}   Unbound allows you to contact Root Servers directly (No Google DNS).${NC}"
...
if [[ "$unbound_choice" == "y" || "$unbound_choice" == "Y" ]]; then
    ...
    echo -e "${BLUE}⏳ Configuring Unbound...${NC}"
...
EOF
    ...
    echo -e "${GREEN}✅ Unbound installed and running.${NC}"
else
...
if [[ "$unbound_choice" == "y" || "$unbound_choice" == "Y" ]]; then
    ...
    echo -e "   Go to Settings > DNS > Custom 1 (IPv4)"
    echo -e "   Enter: ${BLUE}127.0.0.1#5335${NC} and uncheck other DNS."
    echo ""
fi
