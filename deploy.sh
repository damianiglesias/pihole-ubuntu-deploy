...
# Version: 3.1 (Force DNS Config Fix)
# ==========================================
...
EOF
    ...
    # --- MAGIC CONFIGURATION FIX ---
    echo -e "${BLUE}⏳ Forcing DNS settings (Stopping FTL)...${NC}"
    
    # 1. PARAMOS Pi-hole para que no sobrescriba nuestros cambios
    systemctl stop pihole-FTL
    
    # 2. Borramos cualquier DNS anterior del archivo de configuración
    if [ -f /etc/pihole/setupVars.conf ]; then
        sed -i '/^PIHOLE_DNS_/d' /etc/pihole/setupVars.conf
        
        # 3. Inyectamos Unbound como ÚNICO DNS
        echo "PIHOLE_DNS_1=127.0.0.1#5335" >> /etc/pihole/setupVars.conf
    fi
    
    # 4. Arrancamos de nuevo
    systemctl start pihole-FTL
    
    echo -e "${GREEN}✅ Unbound integrated (Google DNS removed).${NC}"
else
...
if [[ "$unbound_choice" == "y" || "$unbound_choice" == "Y" ]]; then
    ...
    echo -e "   Google DNS has been removed from configuration."
fi
